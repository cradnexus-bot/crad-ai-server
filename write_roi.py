    ...
    def run(self, frame, all_rois, prev_gray):
        ...
        for roi_idx, roi_points in enumerate(all_rois):
            ...
            if human_found:
                human_detected = True
                alert_count += 1
                details_parts.append("Human detected in ROI " + str(roi_idx+1))
            ...
            if human_found:
                ...
                x,y = roi_points[0]
                cv2.putText(display,"ROI "+str(roi_idx+1),(x,y),cv2.FONT_HERSHEY_SIMPLEX,0.6,color,2)
                for (x1,y1,x2,y2,conf) in person_boxes:
                    cv2.rectangle(display,(x1,y1),(x2,y2),(0,255,0),2)
                    cv2.putText(display,"Person "+str(round(conf,2)),(x1,y1-8),cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,255,0),2)
                cv2.putText(display,"HUMAN DETECTED - ROI "+str(roi_idx+1),(30,50+alert_count*35),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,0,255),2)
